...
```
タスク完了後のクイックレビューを実行してください。

## 手順
1. .claude/commands/review.md を読み、その手順に従ってレビューを実行
   - 引数なし（クイックレビュー）モードで実行
2. 指摘があれば修正

## 重要
- コミットはしない（全タスク完了後にまとめて行う）
- 軽微な指摘は修正、大きな問題があれば報告
...
### Step 7: ファイナルレビューとまとめてコミット（サブエージェント）
