...
`git diff` で変更ファイルの一覧を取得し、**変更対象ファイルを全て `Read` ツールで読み込んだ上で**以下の観点でチェック:
- 構文エラー、型エラーがないか
...
**A. Claude サブエージェントレビュー:**

`Task` ツールで全変更の最終チェック。変更対象ファイルを全て読み込んだ上でレビュー:
...
## レビュー方法
`git diff` で変更ファイル一覧を取得し、変更対象ファイルを全て `Read` ツールで読み込んだ上でレビューする。
...
両方のレビュー結果を確認し、指摘があれば修正する。修正後はテストが通ることを再確認する。
