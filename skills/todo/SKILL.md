...
### 自動検証（Claude が /todo-run 完了時に確認）
Claudeが「コマンド実行」または「ファイル検索（Glob/Grep）」で合否判定できるものだけ書く。
**そのプロジェクトの具体的なファイル名・メソッド名で書く。** 曖昧な表現は避け、1アクションで検証可能な粒度にする。

- [ ] ビルドが成功する
- [ ] {テストクラス名} が全パスする
- [ ] {ファイル名} に {メソッド名}() が存在する
- [ ] {ファイル名} に /api/xxx エンドポイントが存在する
- [ ] マイグレーション/スキーマ変更ファイルが生成されている

### 手動検証（デプロイ後に人が確認）
外部リソース・UI・実データが必要なものはこちらに書く。
完了時に todo.md の「次にやること」に転記される。
...
### テスト（/todo-run が Red フェーズで実行）
- [ ] AuthServiceのユニットテスト（認証成功/失敗/トークン期限切れ）
- [ ] ログインAPIの統合テスト（正常/不正パスワード/未登録ユーザー）

### 実装（/todo-run が Green フェーズで実行）
- [ ] AuthService追加
- [ ] ログインAPI
- [ ] 認証ミドルウェア
...
**実装ステップの書き方（TDD）:**
- 「テスト」を先に、「実装」を後に書く。これが /todo-run での実行順序になる
- テストには何を検証するかを具体的に書く（「テスト作成」のような曖昧な表現は避ける）
- テストで正解を先に定義し、実装はそれを通すために書く
